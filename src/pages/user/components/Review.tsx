...
const Review: React.FC = () => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      ...
      setError('');
      try {
        const res = await fetch('http://localhost:5118/api/Review');
  ...
  return (
    <div className="bg-gray-50 min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-2xl mx-auto px-2 md:px-0 py-6">
        <h1 className="text-2xl font-bold mb-6 text-orange-500">Đánh giá của bạn</h1>
        {loading ? (
          ...
          <div className="my-6">Bạn chưa có đánh giá nào.</div>
        ) : (
          <div className="space-y-4">
            {items.map(item => (
